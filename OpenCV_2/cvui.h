...
namespace internal
{
	...
	bool radiobox(cvui_block_t& theBlock, int theX, int theY, const cv::String& theLabel, bool *theState, unsigned int theColor) {
		cvui_mouse_t& aMouse = internal::getContext().mouse;
		cv::Rect aRect(theX, theY, 15, 15);
		cv::Size aTextSize = getTextSize(theLabel, cv::FONT_HERSHEY_SIMPLEX, 0.4, 1, nullptr);
		cv::Rect aHitArea(theX, theY, aRect.width + aTextSize.width + 6, aRect.height);
...
bool radiobox(cv::Mat& theWhere, int theX, int theY, const cv::String& theLabel, bool *theState, unsigned int theColor) {
	internal::gScreen.where = theWhere;
	return internal::radiobox(internal::gScreen, theX, theY, theLabel, theState, theColor);
}
